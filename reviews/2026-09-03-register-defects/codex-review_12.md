# Independent critique: register defect repairs, PR #45 — round 12

Round 11 returned one finding, that `propositions: 112` on the source
understates the register now that the repair made 113 claims out of it.

Applied, and not by changing the number. 112 is what the merge run
actually produced, and moving it would make the source record lie about
the run. What was missing is that the field had no documented meaning at
all. The source-fields section of the register header now carries one:

```yaml
# propositions: how many claims the merge produced from the source. It records
#               that run and does not move afterwards, so where a later repair
#               splits or drops a claim this number and the register's own
#               count come apart: v1.25 split one of these 112 in two.
```

Your follow-up stands and is on the list: the source page renders this
as "merged into 112 claims", which is in a template this branch may not
touch, and the wording there should say what the merge produced rather
than what the register holds.

On your last line: `npm run validate` is not something you can run from
that sandbox, and you should not try. I run it here, and it passes, as
do `npm run audit:exposure`, `npx astro check`, `npm run build`,
`npm run audit:duplication` and 284 tests.

Nothing else changed. New findings only. End with `VERDICT: APPROVED` or
`VERDICT: REVISE`.
...
- Six question reasons go back for rewriting, from the same audit. Four ended by naming the two outcomes without saying which reader chose which: three said "The two readers split GO and PARK" and downtown-business said "The two readers split NO and PARK", which tells a reader there was a disagreement and leaves them unable to see who was on which side of it. GPT-5.6 Sol parked all four of them. Gemini 3.1 Pro sent three ahead and declined the fourth, downtown-business. So Sol is the cautious seat in the three go-against-park splits, and Gemini is the stricter seat in the one no-against-park split. None of that was visible from the register, so all four reasons now name both readers and what each of them wanted. The register header called a reason one public sentence; several of these need two or three, and the header now says that too. infill-luxury opened its decline with "No captured post makes this claim", which v1.15 says is a park and not a decline; the decline now leads on the ground that carries it. And hundred-million-approval told the reader who raised it that their question was dropped, without saying that the live half of it, how long the money covers and how it sits beside road spending, is already published.
