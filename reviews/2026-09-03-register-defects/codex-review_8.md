# Independent critique: register defect repairs, PR #45 — round 8, the last

Round 7 returned seven findings. Here is what happened to each, then the
final text. New findings only. End with `VERDICT: APPROVED` or
`VERDICT: REVISE`.

## Round 7, applied

**Standards P1, the version collision.** Fixed. You were right that main
had taken 1.21 through 1.24 while this branch was open. The branch is
merged with main and this entry is now **v1.25**. The rendered entry
below carries that number. Merge order after this one is 1.26 and 1.27
on two other branches.

**Spec P1, the property-tax page still supplies "property".** Applied as
far as the field allows. The question wording and its combined reason
are what two triage seats ruled on and this branch does not rewrite
them. What I could do is stop the page implying she said it, so the
grouping note now says outright that the resident who reported the
increase wrote "my taxes" and not property taxes, which is why the claim
under her name says taxes. That sentence is on the rendered page below.

**Spec P2, "the first audit that looked backwards".** Applied. You were
right that this site had already pulled published claims off its
findings board, which is backwards-looking. The summary now says the
first audit to go back over the register's own decisions, and the full
change note says nothing had gone back over those decisions until that
day.

**Spec P2, four truisms or three.** Applied, and I went with your count
against the audit's. The audit report claims four and names transit
efficiency as the fourth, but that claim's recorded reason is that
"efficient" is undefined, which is a different ground. Only three carry
a reason that says the thing is undisputed. The entry now names those
three and drops the count.

## Round 7, recorded and not applied, with reasons

These three are all real and all outside the files this branch may
change. They are being reported for follow-up rather than silently
dropped, and I would rather you tell me the reasoning is wrong than have
them disappear.

**Standards P2, `prior_triage.readers` accepts a repeated name.** True.
`["gpt-5.6-sol", "gpt-5.6-sol"]` passes. That is a validator change in
`scripts/lib/register-checks.ts` plus a regression test, and this branch
is scoped to the register and the changelog.

**Standards P2, the validator comment and message still say "the
sentence they gave", and the test fixture invents a joint reason.**
True, and it is the same defect I just fixed in the two places I am
allowed to touch. Same files, same reason.

**Spec P1, the claim id `property-taxes-rising-sharply`.** You raised
this in round 1 and again in round 7 and I agree with you both times.
Renaming it needs a line in `public/_redirects`, which this branch may
not touch. It is the first item on the follow-up list.
