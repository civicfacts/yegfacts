# Independent critique: register defect repairs, PR #45 — round 10

Round 9 returned two findings. Both applied.

**Standards P2, the one-sentence contract.** You were right that the
header promised one sentence and six reasons deliver two to four. I
changed the contract rather than compressing, because the extra
sentences are the disclosure you spent four rounds getting into them,
and the change is now recorded in the v1.25 entry. The field now reads:

```yaml
# reason:         why triage answered as it did, in public. One sentence
#                 where one will do, and two or three where the readers split
#                 and a reader has to be told who wanted what. Required on
#                 park and no — the register is published, so a declined
#                 question never sits here unexplained.
```

**Spec P2, "not the sides of it".** You were right, GO and PARK are the
sides. It now says the four reasons named the two outcomes without
saying which reader chose which.

Nothing else changed. The register header's `accounts` and
`prior_triage` paragraphs and the six reasons are unchanged from round
9. The v1.25 entry as it now stands is below, with the record after it.
New findings only. End with `VERDICT: APPROVED` or `VERDICT: REVISE`.
