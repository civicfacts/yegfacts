# Independent critique: register defect repairs, PR #45 — round 7

Round 6 returned two findings and both were right. Both applied.

**P1, false consensus on the property-tax limit.** You were right and
this was the same defect the branch exists to fix, committed by me in
the repair. Gemini 3.1 Pro wrote only "The City's property tax rate
history and assessments can show the typical increase for residents."
The caution about the residents' own notices is GPT-5.6 Sol's alone and
reached the register through the combined reason. The grouping note no
longer says both readers put that limit on it.

**P2, the misquoted split sentence.** Applied. Three said GO and PARK;
downtown-business said NO and PARK, and the sentence now says so.

Nothing else changed. Same rules, same labels. New findings only. End
with `VERDICT: APPROVED` or `VERDICT: REVISE`.
...
One resident's increase and another's yearly bill are different measures of the same complaint about what Edmontonians pay, and both were raised in the same argument. Both readers sent the question ahead on the City's own tax records. Neither resident's own amount can be checked without their assessment and tax notices, which is the limit the combined reason carries.
...
- prior_triage is the schema change, and it is what earns a version. When v1.16 lifted triage from the claim up to the question, the decisions made at the old level went in the bin, and eleven propositions that both triage seats had independently returned NO on became live claims under questions that are going ahead. Four of them are the same undisputed-truism shape that took four published claims off the findings board earlier the same day, and they were queued to be written up and given published findings. The field records the outcome, the seats that reached it, and the reason the run's combined triage file gave for it. On all eleven that reason copies one seat's wording word for word, and the field neither names that seat nor keeps the other seat's reason, which is a gap this version names and does not close. It moves no question's disposition and decides nothing. It means a brief author can see the refusal instead of rediscovering it.
...
### Both seats on `property-taxes`, for checking the grouping note

- Gemini 3.1 Pro, GO: "The City's property tax rate history and
  assessments can show the typical increase for residents."
- GPT-5.6 Sol, GO: "City tax-rate and typical-property records can show
  the general increase, although the two anonymous residents' dollar
  amounts cannot be verified without their assessment and tax notices."
- The run's combined reason is Sol's sentence verbatim, and that is what
  the page prints under "Going ahead".
