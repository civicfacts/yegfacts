# Independent critique: register defect repairs, PR #45 — round 5

Round 4 returned five findings. All five are applied. Same rules, same
labels. End with `VERDICT: APPROVED` or `VERDICT: REVISE`.

## What I did with round 4

**1 (Truth, "an undisputed budget fact").** Applied. You were right that
it answers the declined question without checking it. The reason now
says checking the council vote "would settle nothing anybody here is
arguing about", which is a statement about the source and not about the
council record.

**2 (Promise, what the validator proves).** Applied. The entry now says
the two rules hold the field's shape, and says outright that no rule can
check the values are the ones the run recorded, which is still read by
hand.

**3 (Clarity, authorship).** Applied in both the register header and the
changelog: on all eleven the stored sentence copies one seat's wording
word for word, and the field neither names that seat nor keeps the other
seat's reason.

**4 (Clarity, "private amount").** Applied.

**5 (Plain speech, "a verdict either side could lose").** Applied. The
reason now says that without a definition somebody could be wrong about,
no record could settle it either way.
...
One resident's increase and another's yearly bill are different measures of the same complaint about what Edmontonians pay, and both were raised in the same argument. Neither resident's amount can be checked without that resident's own assessment and tax notices, which is the limit both readers put on the question when they sent it ahead.
