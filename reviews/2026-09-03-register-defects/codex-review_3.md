# Independent critique: register defect repairs, PR #45 — round 3

Round 2 returned nine findings. Here is what happened to each, then the
new text. Same rules, same labels. End with `VERDICT: APPROVED` or
`VERDICT: REVISE`.

## Round 2 findings, applied

**1 (Promise, "so nobody is counted twice").** Applied. You were right
and the paragraph was arguing with itself. The header now says outright
that a double count and a missed person cancel out, so the arithmetic
cannot show each person was counted once.

**3 (Attribution, which seat declined downtown-business).** Applied, and
this was the finding of the round. You caught an error that did not
start with me. The published audit report says "the OpenAI seat parked
all three of the GO-against-PARK splits and declined the one that became
downtown-business", and I copied it. The seat outputs say the opposite:
GPT-5.6 Sol parked downtown-business and Gemini 3.1 Pro declined it. The
changelog now says what the seat outputs say. The audit file is not a
file this branch may change, so its error is being reported separately
rather than quietly patched, and it will not be left standing.

**4 (Truth, the chronology).** Applied. Midday, not morning; that night,
not one afternoon.

**5 (Clarity, "this failure").** Applied.

**6 (Clarity, a panel run is spent on a question, not a claim).**
Applied in three places, in the register header and twice in the
changelog. The right statement of what a hidden refusal costs is a
published finding on a claim two readers turned down, not a run.

**7 (Plain speech, the broken sentence).** Applied.

**8 (Clarity, "right now" in a dated entry).** Applied as "as this is
published", which is fixed by the entry's own date.

**9 (Clarity, "that resident" with two residents).** Applied.

## Round 2 findings, refused

**2 (Attribution, the cycling-safety opening).** Refused, and here is the
reasoning to shoot at. A question's reason is defined in the register as
the register's own public sentence saying why triage answered as it did,
so it speaks in the register's voice by design. What the two seats
actually disagreed about is whether the records can show a lane made
riding safer. That is the sentence that follows, and it concedes Sol's
position before Sol is named. What is left in the opening sentence,
that a traffic bylaw sets who may ride where and the rules that decide
fault, and that collision records show where and how often cyclists are
hit, is not disputed by either seat: Sol's own reason turns on crash
totals lacking exposure data, not on what a bylaw is. If you still think
the opening asserts something a seat denied, quote the seat sentence
that denies it.
...
#                 side somebody who argued on more than one belongs on is an
...
One resident's increase and another's yearly bill are different measures, and both were raised in the same argument about what Edmontonians pay. Neither amount can be checked without the assessment and tax notices of the resident who gave it, which is the reason both readers gave when they sent the question ahead.
...
- prior_triage is the schema change, and it is what earns a version. When v1.16 lifted triage from the claim up to the question, the decisions made at the old level went in the bin, and eleven propositions that both triage seats had independently returned NO on became live claims under questions that are going ahead. Four of them are the same undisputed-truism shape that took four published claims off the findings board earlier the same day, and they were queued to be written up and given published findings. The field records the outcome, the seats that reached it, and the reason the run's combined triage file gave for it, which is one seat's wording rather than a sentence both of them wrote. It moves no question's disposition and decides nothing. It means a brief author can see the refusal instead of rediscovering it.
...
### The four split disclosures, as rendered (unchanged since round 2 except where noted)
...
`cycling-safety` (Going ahead): Edmonton's traffic bylaw sets who may
ride where and the rules that decide fault in a collision, and the
City's collision records show where and how often cyclists are hit.
Neither shows whether a lane made riding safer, because a crash count
with no count of the riders using it cannot tell a more dangerous lane
from a busier one. That is the point the two readers split on. Gemini
3.1 Pro sent it ahead on the strength of those records; GPT-5.6 Sol
parked it until the collision records can be tied to route design and to
how many riders use each route.
...
### The seat outputs, again, for checking the attributions
...
New findings only. Do not re-argue anything settled above unless the
reasoning is bad, and if it is, say which sentence of it is wrong.
